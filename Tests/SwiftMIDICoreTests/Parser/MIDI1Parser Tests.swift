...
struct MIDI1Parser_Tests {
    ...
    private typealias PacketData = MIDIPacketData<Int, String>
    ...
    func packetData_parsedEvents_Empty() {
        ...
        #expect(
            PacketData(bytes: [], timeStamp: .zero)
    ...
    func packetData_parsedEvents_SingleEvents() throws {
        ...
        func parsedEvents(bytes: [UInt8]) -> [MIDIEvent] {
            PacketData(bytes: bytes, timeStamp: .zero)
                .parsedEvents(using: parser)
        }
