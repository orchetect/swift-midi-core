//
//  ThreadSynchronizedPThreadMutex Variable Tests.swift
...
@Suite struct ThreadSynchronizedPThreadMutex_Variable_Tests {
    private final class Wrapper: Sendable {
        let number: ThreadSynchronizedPThreadMutex<Int>
        
        init(number: Int = 0) {
            self.number = ThreadSynchronizedPThreadMutex(wrappedValue: number)
        }
    }
