...
To increase type safety and enforce value validation, novel value types have been implemented in SwiftMIDI.

## MIDI Events

SwiftMIDI contains a unified set of MIDI events which are seamlessly interchangeable in both MIDI 1.0 and MIDI 2.0 contexts.
...
To account for this, SwiftMIDI introduces unified hybrid value types allowing you to provide the value in the resolution of your choice regardless whether the platform is operating with MIDI 1.0 or 2.0, and SwiftMIDI will scale the value automatically when appropriate.
...
- **Unit Interval (`Double` between `0.0 ... 1.0`)**
  ...
  A floating-point unit interval value is provided specifically in SwiftMIDI as a convenience, and can be used for virtually any MIDI value type. This standardizes values and allows for easier mapping of values to other values.
...
Special attention has been made to ensure midpoints line up across all of these value types, so be sure to always use the enum cases and properties provided by SwiftMIDI as described above to convert values from one format to another.

Internally, values between *min ... midpoint* are calculated differently than values between *midpoint ... max*.

Consider these examples where SwiftMIDI helps you transparently:

- even though a 7-bit MIDI 1.0 value of 64 is considered its midpoint, SwiftMIDI understands that:
